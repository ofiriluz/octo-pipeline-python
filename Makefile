...
build: test isort
	...
	pipenv run python scripts/generate_reqs_for_backends.py
	pipenv run python scripts/generate_setuptools_for_backends.py
...
publish:
	...
	for pkg in $$(find backends_dist/* -iname "*.whl"); do pipenv run twine upload --repository-url https://upload.pypi.org/legacy/ --skip-existing $$pkg; done
...
run:
	...
	pipenv run python -m octo_pipeline_python.octo
