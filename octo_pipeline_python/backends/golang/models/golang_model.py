...
class GolangEntrypointInfo(BaseModel):
    ...
    output_name: Optional[str] = Field(default=None, description='Output name of the entrypoint')


class GolangModel(BaseModel):
    go_path: str = Field(default="go")
    linter: Literal['golint', 'golangci-lint'] = Field(default="golangci-lint")
    golint_path: Optional[str] = Field(default=None, description='Path to the linter')
    lint_paths: Optional[List[str]] = Field(default=None, description='Paths to run the linter on, if not given, runs on all')
    ...
    mod_path: Optional[str] = Field(default=None, description='Mod path to use instead of root dir one if exists')
